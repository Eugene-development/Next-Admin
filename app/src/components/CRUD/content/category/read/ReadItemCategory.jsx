...
import { is_visible_read_category } from '@/apollo/stores/visible'
import { current_value_category, current_parent_value_category, current_created_category, current_updated_category } from '@/apollo/stores/current'

const ReadItemCategory = () => {
    const visibleForm = useReactiveVar(is_visible_read_category)
    ...
    const currentCreatedCategory = useReactiveVar(current_created_category)
    const currentUpdatedCategory = useReactiveVar(current_updated_category)
    const cancelButtonRef = useRef(null)

    return (
        <>
            <Transition.Root show={visibleForm} as={Fragment}>
            <Dialog as="div" className="relative z-10" initialFocus={cancelButtonRef} onClose={() => is_visible_read_category(false)}>
                <Transition.Child
                as={Fragment}
                        ...
                        <div className="mt-3 text-center sm:mt-5">
                            ...
                            <div className="mt-8 sm:mt-10">
                                <button
                                    ...
                                    onClick={() => is_visible_read_category(false)}
                                    ref={cancelButtonRef}
                                    >
                                    Закрыть
                                </button>
...
export default ReadItemCategory
